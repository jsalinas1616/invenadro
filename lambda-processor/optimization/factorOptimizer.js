...
const optimizarFactorRedondeo = async (datos, factorInicial, configReglas, inversionOriginal = 0) => {
  ...
  console.log(`[TERNARY] ⬆️  Expansión arriba: +${EXPANSION_ARRIBA} = ${factorInicial} + ${EXPANSION_ARRIBA} = ${(factorInicial + EXPANSION_ARRIBA).toFixed(2)}`);
  console.log(`[TERNARY] ⬇️  Expansión abajo: -${EXPANSION_ABAJO} = ${factorInicial} - ${EXPANSION_ABAJO} = ${(factorInicial - EXPANSION_ABAJO).toFixed(2)}`);
  ...
  while (right - left > PRECISION_BUSQUEDA) {
    ...
    if (mid1 >= mid2 || mid1 === left || mid2 === right) {
      console.log(`[TERNARY] ⚠️ Convergencia alcanzada: puntos coinciden [${left}, ${right}]`);
      break;
    }
    ...
    if (eval1.diferencia <= eval2.diferencia) {
      ...
      right = mid2;
      console.log(`[TERNARY] ➡️ Eliminando tercio derecho, nuevo rango: [${left.toFixed(2)}, ${right.toFixed(2)}]`);
    } else {
      // El mínimo está en la mitad derecha o centro  
      left = mid1;
      console.log(`[TERNARY] ⬅️ Eliminando tercio izquierdo, nuevo rango: [${left.toFixed(2)}, ${right.toFixed(2)}]`);
    }
  }
